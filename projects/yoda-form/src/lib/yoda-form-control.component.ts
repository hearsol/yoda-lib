...
export type YodaFormType =
  'row' | 'subtitle' | 'template' | 'text' | 'textarea' |
  'date' | 'number' | 'decimal' | 'typeahead' | 'file' | 'file-multiple' |
  'checkbox' | 'radio' | 'radio-h' | 'select' | 'search-list' | 'button';
...
export class YodaFormControlComponent implements OnInit, OnDestroy, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['reload']) {
      if (this._subscribers.reload) {
        this._subscribers.reload.unsubscribe();
        this._subscribers.reload = null;
      }
      if (this.reload) {
        this._subscribers.reload = this.reload.subscribe(res => {
          if (this.options) {
    ...
    if (changes['refresh']) {
      if (this._subscribers.refresh) {
        this._subscribers.refresh.unsubscribe();
        this._subscribers.refresh = null;
      }
      if (this.refresh) {
        this._subscribers.refresh = this.refresh.subscribe(res => {
          this.refreshState();
        });
      }
    }
